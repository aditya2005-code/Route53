...
export default function DeleteHostedZoneModal({
  ...
  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      await hostedZoneService.deleteHostedZone(zone.id);
      showToast("Hosted Zone deleted successfully.", "success");
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error("Delete hosted zone failed:", err);
      const msg = err.response?.data?.detail || "Failed to delete hosted zone. Please try again.";
      showToast(msg, "error");
    } finally {
      setIsDeleting(false);
    }
    ...
    >
      ...
      <div className="w-full max-w-[440px] bg-white border border-[#d5dbdb] shadow-xl rounded-sm flex flex-col">
        ...
        <div className="flex items-center justify-between border-b border-[#eaeded] px-6 py-4">
          <h2 id="delete-zone-title" className="text-sm font-bold text-[#d13212] flex items-center gap-1.5">
        ...
        <div className="p-6 space-y-3 text-xs text-[#111111] leading-relaxed">
          ...
          <div className="border border-red-100 bg-red-50/40 p-3 text-zinc-700 space-y-1.5 rounded-sm">
            <div>
              <span className="font-bold text-zinc-900">Domain name:</span>{" "}
              <span className="font-mono bg-white px-1.5 py-0.5 border border-zinc-200 rounded-sm">{zone.domain_name}</span>
            </div>
            <div>
              <span className="font-bold text-zinc-900">Hosted zone ID:</span>{" "}
              <span className="font-mono bg-white px-1.5 py-0.5 border border-zinc-200 rounded-sm">{zone.id}</span>
            </div>
          </div>
          <p className="text-zinc-500">
            <span className="font-bold text-[#d13212]">Warning:</span> Deleting this hosted zone will permanently delete the zone and all of its associated DNS records. This action cannot be undone.
          </p>
        </div>
